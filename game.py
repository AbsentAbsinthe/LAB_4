...
def remove_punct(text):
    ...
    text_without_punc = ""
    for x in range(0, len(list_of_text)):
        if (list_of_text[x].isalpha() or list_of_text[x].isdigit() or list_of_text[x] == " "):
            text_without_punc = text_without_punc + list_of_text[x]
...
def print_menu(exits):
    ...
    possible_directions = list(exits.keys())

    for i in range(0, len(possible_directions)):
        current_direction = possible_directions[i]
        possible_room_name = rooms[exits[current_direction]]["name"]
        print_menu_line(current_direction, possible_room_name)
...
def is_valid_exit(exits, user_input):
    ...
    """
    possible_directions = list(exits.keys())
    if user_input in possible_directions:
        return True
    else:
        return False
...
def menu(exits):
    ...
    while True:
        ...
        users_choice = normalise_input(users_choice)
        
        if is_valid_exit(exits, users_choice):
            return(users_choice)
        else:
            print("Sorry this is not a valid input, your options are:")
